...
-   `lambda` (匿名函数) 用例 —— 函数式编程:
    -   常用于 list 的多种方法
        ...
        -   `list(map(lambda x:x的表达式,<list>))` 
            重写每个元素(注意map返回`<iterable>`), 因此必须用 `list` 将其实体化. 用于扁平输出的话, `', '.join(map())` 也不错.
...
### 包
-   利用 `request` 进行网络请求(爬虫/客户端交互)时, 尽可能创建并复用 `TCP Session`, 减少握手时间开支!
    另一个常见请求失败的原因是没有配置 `User-Headers`
    ```python
...
-   特殊方法 - 每个类自行定义基于该语言运算符的特定行为
    ...
    `__cmp__, __call__, __add__, __sub__, __mul__, __truediv__, __mod__, __pow__,` - 基本运算(同样支持重写), 经典例子是 numpy 的布尔索引.
-   对象类型判断
    ```python showLineNumbers
    type(a) -> <class 'int'>
...
-   `<sequence>[i]` 调用 `.__getitem__()` 方法  
    `<sequence>[a:b]` 标记创建 slice 对象, 对 ndarray 则是 viewport
...
-   `try: except type as e: else: finally:` 未发生异常 else, finally 不管有没有异常都会执行.
## Git Hacks
...
-   而对于 `\docs` 下的多个分类, 有必要生成不同的 Sidebar 以精简展示内容时, 直接使用 `autogenerated` 便会缺失分类本身对应的 item. 这时应当混合使用固定和自动生成:
    ...
    apiSidebar: [{
      ...
      link: {
        ...
        description: 'Learn about the most important api concepts!', //使用 description 提供自动生成页面的内容
    ...
    ```
- 对分类起始页, 可以选择`type: generated-index`, 也可以使用自己的 markdown 文档, 需要使用`link: {type: 'doc', id: 'index'}`属性 并在子目录创建对应的 `index.md`. 如果仍然需要展示分类的 `generated-index`, 可以利用 MDX 特性添加如下代码:
  ```jsx
  <BrowserWindow>
